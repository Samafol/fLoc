...
classdef fLocSession
    ...
    methods
        ...
        function session = run_exp(session, run_num)
            ...
            % Select 6 videos that together will total 6s with ISI
            %videoDurations = session.sequence.all_video_lengths.Duration_Secs;
            %isi = 0.1
            %[validCombination, new_isi] = find_video_combination(videoDurations, 6, 6, isi, 'more_isi');
...
classdef fLocSession
    ...
    methods
        ...
        function session = run_exp(session, run_num)
            ...
            sdc = session.sequence.stim_duty_cycle;
            %stim_dur = session.sequence.stim_dur;
            % Determine stimulus duration per item
            stim_dur = 0.5; % default for images
            stim_name = stim_names{ii};
            if endsWith(stim_name, '.mp4', 'IgnoreCase', true)
                idx = find(strcmp(session.videoData.Filename, stim_name));
                if ~isempty(idx)
                    stim_dur = session.videoData.Duration_Secs(idx);
                else
                    warning('No duration found for video: %s. Defaulting to 1s.', stim_name);
                    stim_dur = 1.0;
                end
            end
            ...
            for ii = 1:length(stim_names)
                ...
                elseif img_ptrs(ii) == -1
                    ...
                    try
                        moviePtr = Screen('OpenMovie', window_ptr, moviePath);
                         Screen('PlayMovie', moviePtr, 1);
                         movieStart = GetSecs;
                         while GetSecs - movieStart < (video_duration + session.sequence.video_isis(ii, run_num)) %stim_dur
                             tex = Screen('GetMovieImage', window_ptr, moviePtr);
                              if tex <= 0
                                  break;
                              end
                ...
                % collect responses
                ii_press = []; ii_keys = [];
                [keys, ie] = record_keys(GetSecs, stim_dur, k);
